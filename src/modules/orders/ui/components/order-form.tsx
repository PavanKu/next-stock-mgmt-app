...
export function OrderForm({ onSuccess, onCancel, initialValues }: Props) {
  const trpc = useTRPC();
  ...
  const form = useForm<z.infer<typeof orderInsertSchema>>({
  ...
  const onSubmit = (values: z.infer<typeof orderInsertSchema>) => {
    if (isEdit) {
      updateOrder.mutate({...values, id: initialValues.id});
    } else {
      ...
      <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-between gap-x-2">
          ...
          <Button type="submit" disabled={isPending}>
            {isPending ? (isEdit ? "Updating..." : "Creating...") : (isEdit ? "Update Order" : "Create Order")}
          </Button>
        </div>
      </form>
    </Form>
