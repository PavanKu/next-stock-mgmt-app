...
// Create a modified schema for purchase orders (without type field)
const purchaseOrderSchema = orderInsertSchema.omit({ type: true });
...
export function PurchaseOrderForm({ onSuccess, onCancel, initialValues }: Props) {
  const trpc = useTRPC();
  ...
  const onSubmit = (values: z.infer<typeof purchaseOrderSchema>) => {
    // Add the hardcoded type for purchase orders
    const orderData = { ...values, type: "purchase" as const };
      ...
      <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
                        ...
                        <FormControl>
                          <CommandSelect
                            ...
                            onSelect={(value) => {
                              ...
                              // Auto-fill cost price for purchases
                              const product = products.data?.items.find(
                                (p) => p.id === value
                              );
                              if (product) {
                                const price = parseFloat(product.costPrice || "0");
        ...
        <div className="flex justify-between gap-x-2">
          ...
          <Button type="submit" disabled={isPending}>
            {isPending ? (isEdit ? "Updating..." : "Creating...") : (isEdit ? "Update Purchase Order" : "Create Purchase Order")}
          </Button>
        </div>
      </form>
    </Form>
